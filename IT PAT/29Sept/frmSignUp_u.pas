...
uses frmRoder_u, frmLogIn_u, frmSplashScreen_u;
        ...
        if (sUserName[Length(sUserName)] in ['1'..'9'])
          then
            begin
              ...
              //Stop die code want daar is n error
              Exit;
            end
          else
...
//Check of die password korrek is
  ...
  if Length(sPassword) >= 8
    then bVlag2 := true
      ...
      begin
         MessageDlg('The Password is too short', TMsgDlgType.mtInformation, [mbOK, mbCancel], 0);
  ...
  if (sPassword[1] in ['A'..'Z'])
    then bVlag3 := true
      ...
      begin
        Showmessage('Password must begin with a capital letter');
  ...
  if sPassword[Length(sPassword)] in ['1'..'9']
    then bVlag4 := true
      ...
      begin
        ShowMessage('Password must end with a number');
  ...
  if bVlag1 and bVlag2 and bVlag3 and bVlag4 and bVlag5
    then
      begin
        ...
        dmData.ADOTableUsers.Append;

        // Add die nuwe waardes na die database se regte fields
        dmData.ADOTableUsers.FieldByName('UserName').AsString := sUsername;
        dmData.ADOTableUsers.FieldByName('Email').AsString := sEmail;
        dmData.ADOTableUsers.FieldByName('Password').AsString := sPassword;
...
var
  ...
  clPrimaryClr, clLightBlueClr : TColor;
