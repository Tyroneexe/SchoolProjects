...
var
  ...
  primaryClr, lightBlueClr : TColor;
...
var
  ...
  sNewPassword, sHoofAlfabet, sAlfabet : string;
  iConfirmNewPassword : integer;
  I: Integer;

begin

  sAlfabet := 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  sHoofAlfabet := 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  sEmail := edtEmail.Text;
              ...
              if sEmail = dmData.ADOTableUsers.FieldByName('Email').AsString
                then
                  begin
                    ...
                    //Gee die user n ID sodat ons die user later kan vind
                    dmData.sUserID := dmData.ADOTableUsers.FieldByName('ID').AsString;

                    //Ask to generate password here
                    if MessageDlg('Do you want to Generate a Password', TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbCancel], 0) = mrCancel
                      then
                        begin 
                        ...
                        if (sNewPassword[1] in ['A'..'Z'])
                          ...
                          else //user will die password generate
                            begin
                              //Add n random Hoofletter eerste
                              sNewPassword := sHoofAlfabet[randomrange(1,27)];

                              //dan ad ons n nommer
                              sNewPassword := sNewPassword + floattostr(trunc(pi * random(10)));

                              //-7 want ons wil die 8ste karakter n nommer he
                              for I := 1 to 7 - length(sNewPassword) do
                              begin
                                //Add n nommer
                                if length(snewpassword) mod 2 = 0
                                  then //ewe getal
                                    sNewPassword := sNewPassword + inttostr(randomrange(1,10))
                                  //dan add n letter
                                  else //onewew getal
                                    sNewPassword := sNewPassword + sAlfabet[randomrange(1,53)];
                              end;

                              //Password moet eindig met n nommer so ons add n random nommer
                              snewpassword := snewpassword + inttostr(floor(random(4) * pi));

                              //Laat die user weet
                              showmessage('Your new password is : ' + sNewPassword);
