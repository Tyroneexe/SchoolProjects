...
begin
  with dmData do
        begin
          ...
          qryUsers.sql.Text := 'select * from tblUsers where isAdmin = True';
...
//  for I := 1 to length(semail) do
 ...
 with dmData do
    begin
      qryUsers.Active := false;
      qryUsers.sql.Text := 'select * from tblUsers where Epos like ' + QuotedStr('%' + semail + '%');
      qryUsers.Active := true;
    end;
end;
...
begin
  ...
  then
    begin
      ...
      begin
        qryUsers.Active := false;
        qryUsers.sql.Text := 'select * from ' + sdbGrid + ' order by ' + scolumname + ' DESC';
        qryUsers.Active := true;
      end;
    end
    else
      begin
        ...
        begin
          qryUsers.Active := false;
          qryUsers.sql.Text := 'select * from ' + sdbGrid + ' order by ' + scolumname + ' ASC';
          qryUsers.Active := true;
        end;
      end;
end;
...
begin
  ...
  begin
    ...
    begin
      qryUsers.Active := false;
      qryUsers.sql.Text := 'select * from ' + sdbgrid;
      qryUsers.Active := true;
    end;
  end
  else
