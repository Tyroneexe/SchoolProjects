...
type
  TfrmHealth = class(TForm)
    ...
    procedure CreateUserInfoContainer();
    procedure GetUserDetails();
    procedure LoadNavBarIcons();
    procedure ChangeNavBarColor();
    procedure LoadWorkoutPage();
    procedure PreviousPage();
    procedure AddCaloriesClick(Sender: TObject);
    procedure LoadStatsPage();
    procedure LoadFatLossPage();
    procedure LoadFatLossHealthData();
    procedure LoadImprovements();
    procedure GetNotifications();
    procedure LoadTrackingPage();
    procedure AddRecipe(Sender: TObject);
    procedure GoToSettings(Sender: TObject);
    procedure LoadTodayCalories();
    procedure GraphShapeNoDataStats();
    //
...
procedure TfrmHealth.LoadWorkoutPage();
var
...
begin
  ...
  GetNotifications();
  LoadWorkoutPage();
  LoadFatLossPage(); //As workout program verander moet dit rebuild
  LoadStatsPage();
  LoadTrackingPage();
  LoadTodayCalories();
...
var
  ...
  arrFoundGraph : array[1..7] of boolean;
...
begin
  ...
  begin
    ...
    begin
      if tblHealthData['GebruikerID'] = strtoint(sUserID)
        ...
        begin
          ...
          if tblHealthData['Datum'] = (date - 6)
            then
              begin
                if tblHealthData['Calories'] <> 0 then
                begin
                ...
                arrFoundGraph[1] := True;
              end;


          if tblHealthData['Datum'] = (date - 5)
            then
              begin
                if tblHealthData['Calories'] <> 0 then
                begin
                ...
                arrFoundGraph[2] := True;
              end;


          if tblHealthData['Datum'] = (date - 4)
            then
              begin
              if tblHealthData['Calories'] <> 0 then
                begin
                ...
                arrFoundGraph[3] := True;
              end;


          if tblHealthData['Datum'] = (date - 3)
            then
              begin
              if (tblHealthData['Calories'] <> 0) then
                begin
                ...
                arrFoundGraph[4] := True;
              end;


          if tblHealthData['Datum'] = (date - 2)
            then
              begin
                if (tblHealthData['Calories'] <> 0) then
                begin
    ...
    if not arrfoundGraph[1] then shpGraph1.Height := 10;
    if not arrfoundGraph[2] then shpGraph2.Height := 10;
    if not arrfoundGraph[3] then shpGraph3.Height := 10;
    if not arrfoundGraph[4] then shpGraph4.Height := 10;
    if not arrfoundGraph[5] then shpGraph5.Height := 10;
    if not arrfoundGraph[6] then shpGraph6.Height := 10;
    if not arrfoundGraph[7] then shpGraph7.Height := 10;
  end;

  GraphShapeNoDataStats();
end;
    ...
    begin
      if tblHealthData['GebruikerID'] = strtoint(sUserID)
        ...
        begin
          ...
          then
          begin
            if tblHealthData['SleepHours'] <> 0 then
            begin
...
begin
 ...
 with dmData do
  begin
    ...
    while not tblHealthData.Eof do
    begin
      if tblHealthData['GebruikerID'] = strtoint(sUserID) then
      begin

      if tblHealthData['Datum'] = (date - 6) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date - 5) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date - 4) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date - 3) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date - 2) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date - 1) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
      ...
      if tblHealthData['Datum'] = (date) then
        begin
          if tblHealthData['Steps'] <> 0 then
          begin
...
procedure TfrmHealth.ChangeNavBarColor();
begin
...
procedure TfrmHealth.CreateUserInfoContainer();
var
...
begin
  ...
  //Format Rich Edit
  redtRecipesMade.SelAttributes.name := 'Roboto';
  redtRecipesMade.SelAttributes.Size := 18;
  redtRecipesMade.SelAttributes.Style := [TFontStyle.fsUnderline];
  redtRecipesMade.lines.Add(sname + ' made:');
...
procedure TfrmHealth.GetUserDetails();
var
      ...
      if itelline = ilineid
        then
          begin
            ...
            delete(sline, 1, length(sfoodname) + 1);
            singredients := copy(sline, 1, pos('#', sline) - 1);
...
procedure tfrmHealth.LoadNavBarIcons();
begin
...
procedure TfrmHealth.PreviousPage();
var
...
procedure TfrmHealth.LoadFatLossPage();
var
...
begin
  ...
  //Load Health data
  LoadFatLossHealthData();
  LoadImprovements();
end;

procedure TfrmHealth.LoadFatLossHealthData();
var
...
procedure TfrmHealth.LoadImprovements();
var
...
procedure TfrmHealth.GetNotifications();
var

  bfound : boolean;
  snotify, sline : string;
  ipos, itelNotifys, ikode : integer;
    ...
    while (not tblusers.Eof) and (bfound = false) do
      begin
        if tblUsers['GebruikerID'] = suserid
          then
            begin
              if (not VarIsNull(tblUsers['Notification'])) and (tblUsers['Notification'] <> '')
                then
                  begin
...
procedure TfrmHealth.LoadTrackingPage();
var
...
procedure TfrmHealth.LoadStatsPage();
var
      ...
      if tblHealthData['GebruikerID'] = strtoint(sUserID)
        ...
        begin
          if tblHealthData['Datum'] = (date - 6)
            then
            begin
              if tblHealthData['Weight'] <> 0 then
              begin
...
procedure TfrmHealth.LoadTodayCalories();
begin
...
procedure TfrmHealth.GraphShapeNoDataStats();
begin
  if shpGraph1.Height = 10
    then
      begin
        shpGraph1.Height := 10;
