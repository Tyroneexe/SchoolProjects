    ...
    while (not tblUsers.Eof) and (bfound = false) do
      begin
        if tblUsers['Naam'] = susername
          then
            begin
              dbgUsers.FieldCount; //gaan na die ry (moet dalk sql gebruik)
              bfound := true;
            end;
  ...
  if MessageDlg('Do you want to send a notification to all users?', TMsgDlgType.mtConfirmation, [mbNo, mbAll], 0) = mrAll
    ...
    begin
      ...
      begin
        ...
        showmessage(suser + 'iuser: ' + inttostr(iuser));
              ...
              if bSearchName
                ...
                begin
                  if uppercase(tblUsers['Naam']) = uppercase(suser)
                    then
                    begin
                      snotification := inputbox('Notification','What message do you want the user to see','');
                      tblUsers.Edit;
                      tblUsers['Notification'] := snotification;
                      tblUsers.Post;
        ...
        if tblUsers['GebruikerID'] = strtoint(sUserID)
          then
            begin
              ...
              //Position
              lblName.Left := (pcPages.Width div 2) - (lblName.Width div 2);
